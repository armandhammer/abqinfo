...
def pdf_metrics(p):
    ...
    return count,total
def main():
    decision=load(DECISION); inventory={r['id']:r for r in load(INVENTORY)['candidates']}; r2={r['key'] for r in load(R2)['objects']}; records=[]
    for index,item in enumerate(sorted(decision['records'],key=lambda x:x['series_order'])):
        ...
        visual='pending_host_word_render_and_human_visual_inspection' if is_docx else 'passed_all_pages_rendered_and_contact_sheet_reviewed'
        detail=('Codex Word COM is unavailable (HRESULT 0x80070520). The supplied host-side helper verifies staged bytes, opens each original read-only, and exports inspection-only PDFs; no DOCX conversion or source mutation occurred.' if is_docx else f'All {pages} pages rendered successfully; contact-sheet review found no missing/corrupt pages or obvious visual defects.')
        ...
        candidate=inventory[item['id']]; note=f'Archive preparation 2026-09-21: re-fetched authoritative NMDOT original matched {size} bytes and SHA-256 {checksum}; proposed R2 key {key}; no R2 upload or public-byte verification performed.'; notes=candidate['processing_notes']+([] if note in candidate['processing_notes'] else [note]); inventory_status=('archive preparation complete except pending DOCX host-side visual inspection; no R2 archive or public-byte verification performed' if is_docx else 'archive preparation complete with PDF render/visual QA passed; no R2 archive or public-byte verification performed')
    ...
    artifact={'schema_version':1,'artifact_type':'nmdot_statewide_truck_parking_study_archive_preparation','recorded_at':'2026-09-21','state':'archive_preparation_substantially_complete_pending_docx_host_visual_inspection_no_r2_mutation','source_decision':str(DECISION.relative_to(ROOT)).replace('\\','/'),'scope_candidate_ids':[r['id'] for r in records],'series':{'name':'NMDOT statewide truck-parking study','relationship':'Complete ordered seven-part series; do not split or substitute any part.','total_source_bytes':total},'records':records,'summary':{'source_verification_passed':7,'docx_visual_qa_pending':5,'pdf_render_qa_passed':2,'r2_key_collisions':0,'total_source_bytes':total},'host_docx_renderer':{'helper':'scripts/project/Render-NmdotTruckParkingDocxInspection.ps1','staged_originals':STAGING,'current_codex_limitation':'Word COM unavailable in Codex Windows execution context: HRESULT 0x80070520; this is not a source-document defect.','output_report':f'{STAGING}/docx-visual-render/host-docx-render-report.json'},'remaining_gates':['Five DOCX visual inspections remain pending through the supplied host-side helper.','No R2 upload/storage mutation is authorized.','Exact public-byte verification is required only after a separately authorized upload.','No Hugo, PR, merge, or deployment action is authorized.'],'safeguards_observed':{'r2_mutation':False,'public_content_changed':False,'merge_or_deploy':False}}
