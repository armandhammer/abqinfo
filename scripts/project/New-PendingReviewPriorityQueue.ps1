...
$ranked = foreach ($candidate in @($inventory.candidates | Where-Object status -eq 'pending review')) {
  ...
  Add-MatchScore $text '(?i)\b(spanish|en espa[nñ]ol|bilingual flyer)\b' -14 'alternate-language or event copy should follow the canonical substantive record' ([ref]$score) $reasons
