...
def main() -> None:
    ...
    validated = closeout_path.exists() and load(closeout_path).get('production_verification_result') == 'passed'
    ...
    for prep_row, result, key, size, checksum in zip(prep['records'], evidence['results'], KEYS, SIZES, HASHES):
        ...
        assert row['status'] == ('validated' if validated else 'implemented' if implemented else 'placement assigned')
    ...
    print('later-MS4 archive: six exact public objects, 426,926,738 added bytes, R2 1,237 / 9,218,281,842, ' + ('validated live' if validated else 'implemented on branch' if implemented else 'placement assigned'))
