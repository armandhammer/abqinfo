...
`src-05b68a5758490499` is complete as `requires human review`: its saved City-source research records the unresolved choice to publish, redact, or summarize named residents' fiber-rulemaking correspondence. It has no R2 object or content placement. The 2014 MS4 Annual Report package decision is complete locally: exactly one main body and 27 attachments (28 records total), with saved exact-source hashes in `project-state/discovery/2014-ms4-package-decision-2026-09-18.json`. All 28 remain pending review behind explicit archive/publication and targeted contact/complaint-detail review gates; do not treat attachments as individual review items. Although the generated `next_pending_id` remains MS4 attachment `src-05ec421cb265b29a`, ordinary work must skip that externally gated package as a family. `project-state/discovery/ordinary-queue-terminal-integration-batch-2026-09-19.json` closes 30 final-sweep terminal records, and `project-state/discovery/ordinary-queue-terminal-integration-batch-2026-09-19-parks-web-pages.json` closes all 38 saved Parks/Recreation live-HTML page records. The 17 recreation-paperwork exclusions, six archive-gated approved records, two duplicate records, and 34-record Prescription Trails requires-human-review package remain untouched. `src-06d0fc4cd0abdef6` is now `approved for addition` as a unique 15-page August 2026 Mayor's State of the City / theory-of-change address; the review is `project-state/discovery/theory-of-change-review-2026-09-19.json`. It has no appropriate existing canonical page: do not use ABQInfo's About page or City Progress & Surveys page. Its separate R2 archival, public-byte verification, and City-government/policy placement gates remain unapproved. The next ordinary-queue candidate is `src-070a763aa9701f86` (the saved Council administration-questions-and-answers candidate); do not begin it without a new task instruction.
