...
The post-MS4 filtered queue is recorded in `project-state/discovery/ordinary-queue-next-position-2026-09-20-post-ms4.json`. Six pending records remain outside durable completed/gated scopes; the next genuinely actionable family is the one-record Council Cruising Task Force final report beginning `src-59a9ee7af1987815`, based on saved research only. Do not research it until separately instructed. The two public-service guides, two Form Based Zones parts, and one UNM 2040 framework remain later residual clusters.
