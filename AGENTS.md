...
- Write PR descriptions as human-readable UTF-8 Markdown: use actual newline characters, never literal `\n` escape sequences or command-output prose. Start with a concise plain-English summary, then separate what changed, archive/inventory/provenance, visible site changes, validation, and deferred work. For every visible change, name the exact public page URL, heading, and document title; counts alone are not sufficient.
